...
public class PacienteController {
    ...
    @PostMapping
    @Transactional
    public ResponseEntity cadastrar(@RequestBody @Valid DadosCadastrosPaciente dadosCadastrosPaciente, UriComponentsBuilder uriBuilder){
        var paciente = new Paciente(dadosCadastrosPaciente);
    ...
    public ResponseEntity<Page<DadosListagemPaciente>> listar(@PageableDefault(size = 10, sort = {"nome"}) Pageable pageable) {
        // Paginação dos retornos dos médicos, aqui colocamos um page e fazendo um findBy se ele é ativo ou não, mapeando como aquele DTO para colocar na variável page
